...
impl EspMqttSink {
    ...
    pub fn connect(
        ...
        // Event pump: MUST run, otherwise the client blocks. Forwards command publishes as
        // intents (the loop authorises fail-closed). Ends when the client is dropped.
                                ...
                                Ok((cmd, pin)) => {
                                    ...
                                    let cmd = match (cmd, area) {
                                        (telenot_app::BridgeCommand::Arm(a), Some(id))
                                            if id != 1 =>
                                        {
                                            telenot_app::BridgeCommand::ArmArea { cmd: a, area: id }
                                        }
                                        (c, _) => c,
                                    };
                                    app.lock()
        ...
        let _ = client.subscribe(&cmd_topic, QoS::AtLeastOnce);
        let area_cmd_topic = format!("{root}/area/+/command");
        let _ = client.subscribe(&area_cmd_topic, QoS::AtLeastOnce);
        log::info!("MQTT verbunden: {url} (subscribe {cmd_topic}, {area_cmd_topic})");
        Ok(Self {
            root,
            client: Some(client),
        })
