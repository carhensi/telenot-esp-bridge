...
fn ema_loop(
    ...
    // Loop-Takt (Host-only): 1 Scan-Query pro Tick → TELENOT_SIM_TICK_MS steuert das
    // Scan-Tempo (Demo-Aufnahmen: kleiner Wert = schneller Scan).
    let tick_ms: u64 = std::env::var("TELENOT_SIM_TICK_MS")
        .ok()
