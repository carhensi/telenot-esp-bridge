...
/// String field slots per record (name, name_ha, topic, location).
const N_STR: usize = 4;
const S_NAME: usize = 0;
const S_NAME_HA: usize = 1;
const S_TOPIC: usize = 2;
const S_LOCATION: usize = 3;
...
impl SensorMut<'_> {
    ...
    pub fn set_location(&mut self, v: &str) -> Result<(), ConfigError> {
        self.set_str(S_LOCATION, v)
    }
...
impl SensorTable {
    ...
    /// Bulk construction with location interning (rooms repeat heavily; the
    /// interning map is transient).
    pub fn from_sensors<'a>(
        sensors: impl IntoIterator<Item = &'a Sensor>,
    ) -> Result<Self, ConfigError> {
        let mut t = Self::new();
        let mut locations: std::collections::HashMap<String, (u32, u16)> =
            std::collections::HashMap::new();
        for s in sensors {
            t.push(s)?;
            let idx = t.recs.len() - 1;
            // Re-point the location span at the interned copy (the duplicate
            // just written becomes waste only if it was a repeat).
            if let Some(&(off, len)) = locations.get(&s.location) {
                let rec = &mut t.recs[idx];
                t.arena.waste += rec.lens[S_LOCATION] as usize;
                rec.offs[S_LOCATION] = off;
                rec.lens[S_LOCATION] = len;
            } else {
                let rec = &t.recs[idx];
                locations.insert(
                    s.location.clone(),
                    (rec.offs[S_LOCATION], rec.lens[S_LOCATION]),
                );
            }
        }
        Ok(t)
