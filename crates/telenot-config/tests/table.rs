...
fn sensor(address: u16, name: &str, location: &str) -> Sensor {
    Sensor {
        address,
        name: name.into(),
...
fn tricky_sensors() -> Vec<Sensor> {
    vec![
        sensor(0x0001, "ESG Gehäuse \"vorn\"", "Telenot Zentrale"),
        sensor(0x0042, "Tür \\ Süd / <ÖL>", "Küche"),
        Sensor {
            address: 0x0533,
            name: String::new(),
            name_ha: "🚨 Alarm".into(),
...
fn roundtrip_at_600_sensors() {
    ...
    // Memory bound: interned locations + arena padding must stay well under the
    // naive per-String cost (~600 × 4 × (24 B overhead + payload)).
    assert!(
...
#[test]
fn location_interning_dedupes_rooms() {
    // 200 sensors, 4 rooms: interning keeps the arena near two blocks despite
    // ~200 location copies in the input.
    let sensors: Vec<Sensor> = (0..200)
        .map(|i| {
            sensor(
                i,
                &format!("m{i}"),
                ["EG", "OG", "DG", "Keller"][i as usize % 4],
            )
        })
        .collect();
    let table = SensorTable::from_sensors(&sensors).unwrap();
    assert!(
        table.arena_bytes() <= 16 * 1024,
        "Interning wirkungslos? Arena: {} B",
        table.arena_bytes()
    );
}
