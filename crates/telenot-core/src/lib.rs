...
/// Eager-send idle guard: only fire a command immediately (outside the SEND_NORM poll window)
/// once the line has been idle at least this long since the last received frame. Skips the
/// risky pause right after a panel burst — that window collided reproducibly on the real panel.
/// Tunable after a capture; a collision/no-ACK still falls back to the reliable slot.
pub const EAGER_IDLE_GUARD_MS: Tick = 400;
...
impl Core {
    ...
    fn queue(&mut self, now: Tick, cmd: TxCmd, log: String) -> Vec<Action> {
        ...
        let mut actions = vec![Action::Log(log)];
        // Eager-send (default ON): fire immediately if the line has been idle long enough,
        // instead of waiting for the next ~3 s SEND_NORM poll window. `flush_pending` keeps the
        // single-transaction guard and the pre-arm readiness gate; on collision/no-ACK the
        // existing requeue_on_collision + ACK-timeout retry send it in the reliable slot — so the
        // worst case equals the poll-gated path. See `PanelSettings::eager_send`.
        if self.config.panel.eager_send {
            if let Some(last) = self.last_frame_at {
                if now.saturating_sub(last) >= EAGER_IDLE_GUARD_MS {
                    self.flush_pending(now, &mut actions);
                }
            }
        }
