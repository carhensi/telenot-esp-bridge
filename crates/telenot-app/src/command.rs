...
pub fn homekit_authorized(cmd: BridgeCommand, allow_disarm: bool) -> bool {
...
mod tests {
    ...
    fn arm_and_reset_always_allowed() {
        ...
        for cmd in [
            ...
            assert_eq!(
                authorize_command(cmd, false, None, &mut s),
                CommandAuth::Allow
            );
        }
    }
    ...
    fn bypass_sperren_fail_closed_like_disarm() {
        ...
        assert!(matches!(
            authorize_command(on, false, Some("4729"), &mut s),
            CommandAuth::Deny(_)
        ));
        assert!(matches!(
            authorize_command(on, true, Some("0000"), &mut s),
            CommandAuth::Deny(_)
        ));
        assert_eq!(
            authorize_command(on, true, Some("4729"), &mut s),
            CommandAuth::Allow
        );
    }
    ...
    fn disarm_allowed_only_with_correct_pin() {
        ...
        assert!(matches!(
            authorize_command(BridgeCommand::Arm(ArmCommand::Disarm), true, None, &mut s),
            CommandAuth::Deny(_)
        ));
    }
    ...
    fn homekit_disarm_nur_mit_optin() {
        // Fail-closed: disarm only when explicitly enabled in setup.
        assert!(!homekit_authorized(
            BridgeCommand::Arm(ArmCommand::Disarm),
            false
        ));
