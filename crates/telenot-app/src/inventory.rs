...
mod tests {
    ...
    fn cfg() -> Config {
        let s = |address, name: &str, kind, confirmed, location: &str| Sensor {
            address,
            name: name.into(),
            name_ha: name.into(),
            kind,
    ...
    fn inventory_lists_confirmed_plus_alarm_and_readiness() {
        ...
        assert!(akku.get("area").is_none(), "empty location → no area field");
