...
über die serielle GMS-Schnittstelle ausliest/steuert und über **HomeKit** und **MQTT** anbindet.
...
docs/                    REST-/MQTT-Verträge, Threat-Model, Hardware, GMS-Coverage, HA-Howto, Roadmaps.
reference/               GITIGNORED, optional. Lokaler Ablageort für private Mitschnitte/Echtdaten
                         (s. docs/DEBUG-CAPTURE.md) — existiert im frischen Clone nicht.
```
...
- Verhalten gegen Realität prüfen, NICHT raten: Die Ground-Truth-Fakten realer Mitschnitte
  (pcap + Befehls-Telegramme der Vorgänger-Bridge) sind in der Testsuite kodiert — bei
  Protokoll-Zweifeln gelten die Tests, neue Erkenntnisse nur mit echtem Mitschnitt
  (docs/DEBUG-CAPTURE.md). Reale Hausdaten sind privat → nie committen, nicht in
  Tests/Fixtures hardcoden (Fixtures synthetisch halten).
...
- **Phase A + Host-Appliance**: Protokoll/Core/App/Sim/Setup-Web; Protokoll-Layer vollständig
  gegen reale Mitschnitte verifiziert (pcap + Befehls-Telegramme). Command-Pfad
  HA→Bridge (Arm/Disarm/Bypass/Schaltausgänge, fail-closed + Lockout), GMS-Befehls-Transaktion
  (ACK-Fenster 3 s, max. 3 Retries, fail-safe Verwerfen, **eager-send**: Befehle gehen sofort
  raus statt aufs Poll-Fenster zu warten), Zeitstellen (0x50).
...
- **HomeKit-Direkt-Epic**: HAP nativ (SecuritySystem + Melder per `show_in_homekit`),
  ...
  **XOR-Steuerkanal**: MQTT/REST-Steuerung und HAP schließen sich hart aus — es steuert
  immer genau ein Kanal.
- **A/B-OTA in Betrieb**: SHA-verifizierter Upload, 5-min-Self-Test, Bootloader-Rollback —
  Migration + Rollback-Test am Gerät abgenommen (2026-07). Web-Flasher (esp-web-tools) und
  Release-Workflow (merged Image + OTA-Image + ota.json) stehen.

Offen:

- **Externe Montage** (externe Box mit Sabotagekontakt, UEXT-Stack als Standard-Variante).
...
- **Härtung**: Flash-Encryption / Secure Boot v2 **bewusst verworfen** (2026-07-12) — Restrisiko
