...
| GET | `/sensors` | → `{sensors:[Sensor], rooms:[String], counts:{confirmed,unconfirmed,excluded}}` | S4 |
| PATCH | `/sensors/{addr}` | `{name?,name_ha?,kind?,polarity?,location?,topic?,confirmed?,include?}` → `Sensor` | S4 |
| POST | `/sensors/bulk` | `{addresses:[u16], op, kind?, polarity?, location?}` → `{updated}` | S4 |
...
```json
{
  ...
  "kind": "magnetkontakt", "topic": "mk_haustuer", "location": "Erdgeschoss",
