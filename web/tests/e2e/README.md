...
| Auflösung / Seitenverhältnis | `viewport` je Projekt in `playwright.config.js` |
| Sprache & Theme im Video | `addInitScript` im Spec: `localStorage tn_lang` / `tn_theme` |
| Scan-Tempo | Sim-Loop-Sleep in `serve.rs` (`ema_loop`, aktuell 30 ms) |
