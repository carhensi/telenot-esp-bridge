...
// device (fixture config) → switch to live view first, then the status pill in the
...
export async function openDiagnostics(page) {
  ...
  // Status pill (opens DiagModal via ctx.openDiag) — labeled "Status" in both languages
  // since the Serial pill became the combined status pill.
  const pill = page.getByRole("button", { name: /^Status$/ }).first();
