// Sensor table S4 (core screen): bulk edit, inline edit, drawer, expert editor.
...
// Rooms/areas live in Home Assistant / Apple Home, not in the bridge (HA is authoritative).
...
function buildTSV(list) {
  const head = `# ${"addr".padEnd(6)} | ${"kind".padEnd(4)} | ${"pol".padEnd(3)} | inc | name`;
  const rows = list.map((s) =>
    `${expHex(s.address).padEnd(6)} | ${(KIND2CODE[s.kind]||"?").padEnd(4)} | ${(POL2CODE[s.polarity]||"?").padEnd(3)} | ${(s.include?"y":"n").padEnd(3)} | ${s.name||""}`
  );
  return head + "\n" + rows.join("\n");
}
function buildJSON(list) {
  return JSON.stringify(list.map((s) => ({
    ...
    kind: s.kind, polarity: s.polarity, topic: s.topic, include: s.include,
  })), null, 2);
}
...
function parseEditor(text, fmt, byAddr, t, allByAddr) {
  ...
  const diffsFrom = (cur, next) => {
    const out = {};
    ["name","name_ha","kind","polarity","topic","include"].forEach((k) => {
  ...
  text.split("\n").forEach((raw, i) => {
    ...
    if (cells.length < 5) { errors.push(t("exp.err.cols", { ln })); return; }
    ...
    const name = cells.slice(4).join("|").trim();
    ...
    const next = { name, kind, polarity: pol, include: ic === "y" };
    const d = diffsFrom(cur, next);
    if (Object.keys(d).length) changes.push({ address: addr, ...d });
  });
  return { changes, errors, addrs: [...seen] };
...
function ExpertEditor({ open, onClose, list, all, t, onApply, toast }) {
  ...
  const doCopy = () => { copyText(text); setCopied(true); setTimeout(() => setCopied(false), 1400); };
  const doApply = () => {
...
function SensorTable({ ctx, sensors, setSensors, t, lang, toast }) {
  ...
  // Flat list (rooms are managed in HA/Apple Home, not here). Kept as a single group so
  // the render loops below stay unchanged.
  const groups = useMemo(() => [{ room: "", items: visible }], [visible]);
  ...
  return (
    <div>
      ...
      <SensorDrawer s={drawerSensor} t={t} kl={kl} sensors={sensors}
...
function SensorDrawer({ s, t, kl, sensors, onClose, onPatch, onConfirm }) {
  if (!s) return null;
  const dup = s.dupOf != null ? sensors.find((x) => x.address === s.dupOf) : null;
  return (
    <Modal open={!!s} onClose={onClose} title={t("dr.title")} icon="server" iconTone="accent"
      ...
      <div className="stack stack-4">
        ...
        <Field label={t("dr.nameha")} hint={t("dr.nameha.hint")}><input className="input input--mono" value={s.name_ha} onChange={(e) => onPatch({ name_ha: e.target.value })} /></Field>
