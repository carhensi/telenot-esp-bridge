...
function Callout({ tone = "info", icon, title, children, style }) {
  ...
  return (
    <div className={`callout callout--${tone}`} style={style}>
...
/* ---------- helpers ---------- */
// Clipboard: navigator.clipboard exists only in secure contexts (HTTPS/localhost) — the
// bridge is served over plain HTTP, so fall back to the hidden-textarea execCommand path.
function copyText(txt) {
  if (navigator.clipboard && window.isSecureContext) { navigator.clipboard.writeText(txt); return; }
...
export { Icon, ICON_PATHS, Brandmark, Button, StatusDot, Badge, Checkbox, Toggle, Field, PasswordInput, Modal, Callout, STAT_TONE, copyText, hex, fmtTime, fmtDur, fmtClock, fmtUptime };
